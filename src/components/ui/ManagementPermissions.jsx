...
function PermissionItem({ permission, selected, toggle, search, expandedAll }) {
    ...
    return (
        <div className="ml-4 mb-2">
            <div
                className="flex items-center justify-between border border-gray-300 rounded px-3 py-2 cursor-pointer permission"
...
// ---------------- MANAGEMENT PERMISSIONS COMPONENT ----------------
            ...
            updatePermission.mutate(
                { id: existing._id, ...payload },
                {
                    onSuccess: () => {
                        setToast({
                            show: true,
                            message: "Management permissions updated successfully!",
                            type: "success",
                        });
                    },
                    onError: (err) => {
            ...
            createPermission.mutate(
                { createdBy: adminId, ...payload },
                {
                    onSuccess: () => {
                        ...
                        setToast({
                            show: true,
                            message: "Management permissions created successfully!",
                            type: "success",
                        });
                    },
                    onError: (err) => {
